...
public class FlappyBird implements ActionListener, MouseListener, KeyListener {
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (started) {
			...
			// creating columns infinite times
			for (int i = 0; i < columns.size(); i++) {
				Rectangle column = columns.get(i);
				if (column.x + column.width < 0) {
					columns.remove(column);

					// if this is the top column then add column
					if (column.y == 0) {
			...
			bird.y += yMotion;
			// checking if bird hit the column then game is over
			for (Rectangle column : columns) {
				// adding score here
				...
				if (column.intersects(bird)) {
					...
					if (bird.x <= column.x) {
						...
						// if bird fall the column will take a way the dead bird from the seen
			...
			if (bird.y > HEIGHT - 120 || bird.y < 20) {
